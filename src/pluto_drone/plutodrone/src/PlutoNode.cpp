...
int main(int argc, char **argv)
{
    ...
    if (rc)
    {
     cout << "Error:unable to create communication thread," ;
     exit(-1);
    }
    ...
    if(isSocketCreate)
    {
      cout << "main() : creating write thread, " ;
      rc = pthread_create(&writeThread, NULL, writeFunction,  (void *)2);
      if (rc)
      {
        cout << "Error:unable to create write thread," ;
        exit(-1);
      }
       cout << "main() : creating read thread, " ;
      rc = pthread_create(&readThread, NULL, readFunction,  (void *)3);
      if (rc)
      {
        cout << "Error:unable to read create thread," ;
        exit(-1);
      }
